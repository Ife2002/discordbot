...
export default async function getTotaltokens(Useraddress, nftAddress) {
  try {
    const { body } = await request(
      `https://api.modulenft.xyz/api/v2/eth/nft/collectionsOwned?address=${Useraddress}&count=10&offset=0&type=all&withMetadata=false`
    );
    const responseJSON = await body.json();
    const returned = responseJSON.data;

    let result = returned.filter(
      (collection) => collection.collectionAddress == `${nftAddress}`
    );

...
export default async function getBuyCost(Useraddress, nftAddress) {
  const { body } = await request(
    `https://api.modulenft.xyz/api/v2/eth/nft/userHistory?address=${Useraddress}&contractAddress=${nftAddress}&count=100&offset=0&withMetadata=false&in_type=all&out_type=all`
  );
